...
이 통찰을 *개인 KB*가 아니라 *팀 코드베이스*에 적용하면 — 그게 piki다.
...
**piki는 GitHub Organization의 모든 main 브랜치 변경을 자동으로 흡수해, 코딩 에이전트가 CLI 한 번으로 끌어다 쓰는 팀 컨텍스트 허브를 git repo로 유지하는 시스템이다.**

### 2.2 형태

- 팀은 평소처럼 GitHub Org에 repo들을 운영한다 (변경 없음).
- Org 안에 `wiki`라는 새 repo를 하나 만든다.
- 다른 repo들의 `main`에 push가 일어나면 piki 서버가 webhook을 받는다.
- 서버가 변경 영향 범위를 분석하고, 영향받는 wiki 페이지만 갱신해서 wiki repo에 commit/PR한다.
- 팀원의 Claude Code / Codex / cmux는 작업 시작 시 **SKILL.md**의 정책에 따라 자동으로 `piki` CLI를 호출해 관련 페이지를 끌어온다. 사람도 같은 CLI로 직접 질의 가능.
...
7. **CLI는 보편 인터페이스**. 어떤 에이전트든 `bash`를 실행할 수 있으면 piki를 쓸 수 있다. MCP, IDE 플러그인, 브라우저 확장 같은 환경 종속을 만들지 않는다.

---

## 3. Karpathy llm-wiki와의 관계 및 차별화

piki는 Karpathy 패턴의 *팀 + 코드베이스 + 에이전트* 변형이다.

| | Karpathy llm-wiki | piki |
|---|---|---|
...
- **eliavamar의 `repositories-wiki`**: 단일 레포 + 사람이 읽는 wiki. piki는 멀티 레포 + 레포 간 관계가 1급 + 에이전트 소비 최적화 + push-driven 자동 sync.
- **Devin DeepWiki**: 자동 wiki 생성에서 가장 가까운 제품. 차이는 (1) Org 단위, (2) 에이전트 소비, (3) push-driven incremental, (4) 결과가 git repo로 외화되어 audit/검열 가능.
- **`llms.txt`**: 정적 단일 파일을 사람이 작성. piki는 동적 다파일을 에이전트가 유지.
...
│                       ▼                                  │
├──────────────────────────────────────────────────────────┤
│  Layer 3: Wiki Repo (org/piki, git repo)                 │
│   ├── CLAUDE.md          (스키마)                        │
│   ├── index.md           (카탈로그)                      │
...
- **`piki` CLI**: 에이전트와 사람이 동일하게 쓰는 단일 인터페이스. 환경 종속 없음.
...
2. This wiki: maintained by piki agent + team.
3. Schema: this file.
...
piki의 클라이언트 측은 **두 개의 파일로 끝난다**: `piki` 바이너리 하나, `SKILL.md` 한 장. 어떤 에이전트, 어떤 환경에서도 동일하게 동작.
...
설정 (한 번만):
```bash
piki login          # GitHub OAuth, Org 권한
piki use <org-name> # 활성 Org 선택
```
...
| 명령어 | 시그니처 | 용도 |
|---|---|---|
| `piki ask` | `<question>` | 자연어 질의. 답변 + 출처 페이지 링크. |
| `piki context` | `<file...>` | 작업 중인 파일과 관련된 wiki 페이지 본문을 stdout으로 dump. **에이전트가 가장 자주 부르는 명령**. |
| `piki search` | `<query>` | 키워드 검색. 페이지 경로 + 발췌 목록. |
| `piki read` | `<page-path>` | 특정 페이지 본문 출력 (cat과 비슷). |
| `piki gotchas` | `<repo>` | 해당 repo의 함정 섹션만 골라서 출력. |
| `piki adr` | `[--topic <t>]` | ADR 검색/나열. |
| `piki lint` | (없음) | wiki 헬스체크 (stale, orphan, broken refs). |
| `piki propose` | `--topic <t>` | 새 페이지 작성 제안 (사람이 트리거). |
| `piki status` | (없음) | 마지막 sync 시점, 대기 중 PR 수 등. |
...
[더 보기]: piki read decisions/2026-04-02-payment-v2-migration.md
```

```bash
$ piki context src/payment/refund.ts
...
This codebase is part of an organization that maintains a piki wiki at org/piki.
You MUST use the `piki` CLI before, during, and after non-trivial work.
...
2. Run `piki context <files>` to fetch relevant wiki pages.
...
- An unfamiliar pattern → run `piki search <pattern>` before assuming it's a mistake.
...
- A naming convention dispute → defer to `piki read repos/<repo>/conventions.md`.
- A request to add a new dependency → run `piki search <library-name>`. The team
...
- If you discovered a new gotcha, convention, or design decision that is NOT in the wiki,
  mention it to the user and offer to propose a wiki update via `piki propose`.
...
If `piki context` returns no relevant pages:
...
- Do NOT update wiki pages directly. Use `piki propose` and let the maintainer
...
| Question in natural language  | piki ask "<question>"             |
| Search by keyword             | piki search <query>               |
| Read a known page             | piki read <path>                  |
| All gotchas for a repo        | piki gotchas <repo>               |
| Find an ADR                   | piki adr --topic <t>              |
| Suggest a new wiki page       | piki propose --topic <t>          |
```
...
- **신입 온보딩**: `piki ask "이 회사에서 인증 어떻게 굴러가요?"` → 한 줄로 시작점.
- **PR 리뷰 직전**: `piki context <변경된 파일>` → 리뷰어가 놓칠 수 있는 컨텍스트 한눈에.
- **Slack 대안**: 같은 질문이 반복되는 채널을 CLI 한 번으로 대체.
- **회의 직전**: `piki adr --topic billing` → 어제까지의 결정을 빠르게 복기.
...
- **CLI call rate**: 에이전트 작업 중 `piki context` 호출 비율 (SKILL이 잘 동작하는지 지표)
...
4. **`piki` CLI**: 최소 5개 명령어 (`ask`, `context`, `read`, `gotchas`, `lint`)
...
**[0:30-1:00] piki repo 공개**
- 화면: org/piki repo. `decisions/2026-04-02-payment-v2-migration.md`, `repos/billing-service/gotchas.md`가 이미 존재.
- 내레이션: "piki는 매 push마다 이런 페이지를 자동 갱신해놨다. 사람이 쓴 게 아니라 에이전트가 코드 변경을 보고 컴파일했다."
...
- 에이전트의 행동을 터미널에서 그대로 보여줌:
  ```
  $ piki context src/payment/handlers/
  [관련 페이지 3개 출력]
  ```
- 그 다음 에이전트가 사용자에게 응답:
...
- 30초 안에 piki webhook이 발동되어 wiki repo에 PR이 자동 생성되는 모습.
- 내레이션: "팀이 하던 일을 그대로 하면, wiki는 자동으로 따라간다."

**[2:30-3:00] 마무리**
- 한 줄 요약: *"Compile your team's tribal knowledge into agent-readable context."*
- 한 슬라이드: cmux와의 보완 관계 ("cmux는 *세션 안의 환경*, piki는 *세션을 시작할 때 끌어오는 환경*").
...
A. 걔네는 *코드*를 검색해줌. piki는 *코드에 없는 결정·관습·암묵지*를 외화함. 둘은 보완재. 실제로 에이전트가 둘 다 호출하는 워크플로우도 가능.
...
A. `llms.txt`는 정적/단일/사람작성. 팀 스케일에서 stale을 못 막음. piki는 동적/다파일/에이전트유지 + 사람 게이트.
...
| **Wiki repo** | piki가 유지하는 마크다운 git repo. Org 안에 위치. |
...
| **CLI call rate** | 에이전트가 작업 중 `piki` CLI를 부른 비율. SKILL이 잘 동작하는지 지표. |
| **ADR** | Architecture Decision Record. 시간 순으로 누적되는 결정 기록. |
| **Gotcha** | 코드만 보면 모르는 함정. piki의 가장 가치 있는 페이지 타입. |
...
- **How**: webhook으로 받아서 incremental하게 영향 페이지만 갱신하고, `piki` CLI 한 줄과 SKILL.md 한 장으로 에이전트가 끌어 쓴다.
